...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if input not in cache:
            cache[input]=(diff_function(typed,source,limit),limit)
            return cache[input][0]
        else:
            value,old_limit=cache[input]
            if limit<=old_limit:
                return value
            else:
                cache[input]=(diff_function(typed,source,limit),limit)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if temp<diff:
            res=word
            diff=temp
    return res if diff<=limit else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    def foo(x,y,res):
        if x=="" and y=="":
            return res
        if x=="" or y=="":
            return res+max(len(x),len(y))
        if x[0]!=y[0]:
            res+=1
            if res>limit:
                return res
        return foo(x[1:],y[1:],res)
    return foo(typed,source,0) 
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    i=0
    while i<min(len1,len2) and typed[i]==source[i]:
        i+=1
    res=i/len2
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[]]*len(timestamps_per_player)
    for j in range(len(times)):
        times[j]=[0]*(len(timestamps_per_player[j])-1)
        for i in range(1,1+len(times[j])):
            times[j][i-1]=timestamps_per_player[j][i]-timestamps_per_player[j][i-1]          
    # END PROBLEM 9
